...
public class VxApiRouteHandlerHttpServiceImpl implements VxApiRouteHandlerHttpService {
  ...
  public void handle(RoutingContext rct) {
    ...
    rct.vertx().eventBus().request(thisVertxName + VxApiEventBusAddressConstant.SYSTEM_PLUS_HTTP_API_REQUEST, null);
    ...
    rct.response().endHandler(end -> {
      ...
      // 减少当前正在处理的数量
      rct.vertx().eventBus().request(thisVertxName + VxApiEventBusAddressConstant.SYSTEM_MINUS_CURRENT_PROCESSING, null);
    });
    // 用户请求的request
    ...
    if (policy.isHaveService()) {
      ...
      String reqURL = url += reqQueryParam.toString();
      // 请求处理器
      RequestOptions options = new RequestOptions();
      options
        .setMethod(serOptions.getMethod())
        .setAbsoluteURI(reqURL)
        .setTimeout(serOptions.getTimeOut())
        .putHeader(VxApiRouteConstant.USER_AGENT, VxApiGatewayAttribute.VX_API_USER_AGENT)
        ;
      if (reqHeaderParam != null && reqHeaderParam.size() > 0) {
        options.getHeaders().addAll(reqHeaderParam);
      }
      ...
      httpClient
        ...
        .onSuccess(request -> {
          ...
          request.response().onSuccess(resp -> {
            ...
            LOG.error("API:" + api.getApiName() + ",收到后端服务响应中发生了异常:" + e);
            request.end();
          });
      ...
      vertx.setTimer(serOptions.getRetryTime(), testConn -> {
        ...
        if (service != null) {
          for (VxApiServerURLInfo urlinfo : service) {
            RequestOptions options = new RequestOptions();
            options.setMethod(serOptions.getMethod())
              .setAbsoluteURI(urlinfo.getUrl())
              .setTimeout(serOptions.getTimeOut());
            httpClient.request(options).onSuccess(request -> {
                request.send()
                  .onSuccess(resp -> {
                    ...
                    if (LOG.isDebugEnabled()) {
                      String msg = String.format("应用:%s -> API:%s重试连接后台服务URL->失败:", appName, api.getApiName());
                      LOG.debug(msg, err);
                    }
                  });
              })
