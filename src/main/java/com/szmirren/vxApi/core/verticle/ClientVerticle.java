...
import com.szmirren.vxApi.core.auth.VxApiUserAuthUtil;
...
public class ClientVerticle extends AbstractVerticle {
  ...
  public void staticAuth(RoutingContext rct) {
    ...
    if (user == null) {
      ...
      VxApiUserAuthUtil.isAuthorized(user,VxApiRolesConstant.READ, res -> {
        if (res.succeeded()) {
  ...
  public void staticAPP(RoutingContext rct) {
    User user = rct.user();
    VxApiUserAuthUtil.isAuthorized(user,VxApiRolesConstant.WRITE, res -> {
      if (res.succeeded()) {
        if (res.result()) {
          rct.next();
        } else {
  ...
  public void sysInfo(RoutingContext rct) {
    ...
    vertx.eventBus().<JsonObject>request(thisVertxName + VxApiEventBusAddressConstant.SYSTEM_GET_INFO, null, reply -> {
      if (reply.succeeded()) {
  ...
  public void sysReplaceIpList(RoutingContext rct) {
    ...
    vertx.eventBus().<Integer>request(thisVertxName + VxApiEventBusAddressConstant.SYSTEM_BLACK_IP_REPLACE, param, reply -> {
      if (reply.succeeded()) {
        response.end(ResultFormat.format(HTTPStatusCodeMsgEnum.C200, reply.result().body()));
  ...
  public void findAPP(RoutingContext rct) {
    ...
    // 查询所有APP
    Promise<Message<JsonArray>> findAppFuture = Promise.promise();
    findAppFuture.future().onComplete(res -> {
      if (res.succeeded()) {
    ...
    onlineFuture.future().onComplete(res -> {
      ...
      vertx.eventBus().<JsonArray>request(thisVertxName + VxApiEventBusAddressConstant.DEPLOY_FIND_ONLINE_APP, null, dep -> {
        if (dep.succeeded()) {
    ...
    vertx.eventBus().<JsonArray>request(thisVertxName + VxApiEventBusAddressConstant.FIND_APP, null, findAppFuture);
  }
  ...
  public void getAPP(RoutingContext rct) {
    ...
    if (StrUtil.isNullOrEmpty(name)) {
      ...
      // 查看应用使用已经启动
      Promise<JsonObject> onlineFutrue = Promise.promise();
      onlineFutrue.future().onComplete(res -> {
    ...
    VxApiUserAuthUtil.isAuthorized(user,VxApiRolesConstant.WRITE, res -> {
      if (res.succeeded()) {
        if (res.result()) {
          ...
          vertx.eventBus().<Integer>request(thisVertxName + VxApiEventBusAddressConstant.ADD_APP, param, cres -> {
            if (cres.succeeded()) {
    ...
    VxApiUserAuthUtil.isAuthorized(user,VxApiRolesConstant.WRITE, res -> {
      if (res.succeeded()) {
        if (res.result()) {
          ...
          vertx.eventBus().<Integer>request(thisVertxName + VxApiEventBusAddressConstant.UPDT_APP, param, cres -> {
            if (cres.succeeded()) {
              response.end(ResultFormat.format(HTTPStatusCodeMsgEnum.C200, cres.result().body()));
    ...
    if (StrUtil.isNullOrEmpty(name)) {
      ...
      VxApiUserAuthUtil.isAuthorized(user,VxApiRolesConstant.WRITE, res -> {
        if (res.succeeded()) {
          ...
          vertx.eventBus().request(thisVertxName + VxApiEventBusAddressConstant.DEPLOY_APP_UNDEPLOY, config);
          if (res.result()) {
            // 将应用暂停
            if (vertx.isClustered()) {
            ...
            vertx.eventBus().<Integer>request(thisVertxName + VxApiEventBusAddressConstant.DEL_APP, name, cres -> {
              if (cres.succeeded()) {
                response.end(ResultFormat.format(HTTPStatusCodeMsgEnum.C200, cres.result().body()));
  ...
  public void deployAPP(RoutingContext rct) {
    ...
    if (StrUtil.isNullOrEmpty(name)) {
      ...
      vertx.eventBus().<JsonObject>request(thisVertxName + VxApiEventBusAddressConstant.GET_APP, name, body -> {
        if (body.succeeded()) {
          if (body.result().body().isEmpty()) {
            ...
            vertx.eventBus().<String>request(thisVertxName + VxApiEventBusAddressConstant.DEPLOY_APP_DEPLOY, config, deploy -> {
              if (deploy.succeeded()) {
  ...
  public void unDeployAPP(RoutingContext rct) {
    ...
    if (StrUtil.isNullOrEmpty(name)) {
      ...
      JsonObject config = new JsonObject().put("appName", name);
      vertx.eventBus().<String>request(thisVertxName + VxApiEventBusAddressConstant.DEPLOY_APP_UNDEPLOY, config, deploy -> {
        if (deploy.succeeded()) {
  ...
  public void findAPI(RoutingContext rct) {
    ...
    if (StrUtil.isNullOrEmpty(name)) {
      ...
      vertx.eventBus().<JsonObject>request(thisVertxName + VxApiEventBusAddressConstant.FIND_API_BY_PAGE, body, res -> {
        if (res.succeeded()) {
          vertx.eventBus().<JsonArray>request(thisVertxName + VxApiEventBusAddressConstant.DEPLOY_FIND_ONLINE_API,
  ...
  public void getAPI(RoutingContext rct) {
    ...
    if (StrUtil.isNullOrEmpty(name)) {
      ...
      vertx.eventBus().<JsonObject>request(thisVertxName + VxApiEventBusAddressConstant.GET_API, name, res -> {
        if (res.succeeded()) {
          ...
          vertx.eventBus().<Boolean>request(thisVertxName + VxApiEventBusAddressConstant.DEPLOY_API_IS_ONLINE, isOnLineConf, dep -> {
            if (dep.succeeded()) {
    ...
    VxApiUserAuthUtil.isAuthorized(user,VxApiRolesConstant.WRITE, res -> {
      if (res.succeeded()) {
        if (res.result()) {
          ...
          vertx.eventBus().<Integer>request(thisVertxName + VxApiEventBusAddressConstant.ADD_API, param, cres -> {
            if (cres.succeeded()) {
              response.end(ResultFormat.format(HTTPStatusCodeMsgEnum.C200, cres.result().body()));
    ...
    VxApiUserAuthUtil.isAuthorized(user,VxApiRolesConstant.WRITE, res -> {
      if (res.succeeded()) {
        if (res.result()) {
          ...
          param.put("apiName", dto.getApiName());
          param.put("api", dto.toJson());
          vertx.eventBus().<Integer>request(thisVertxName + VxApiEventBusAddressConstant.UPDT_API, param, cres -> {
            if (cres.succeeded()) {
              response.end(ResultFormat.format(HTTPStatusCodeMsgEnum.C200, cres.result().body()));
      ...
      VxApiUserAuthUtil.isAuthorized(user,VxApiRolesConstant.WRITE, res -> {
        ...
        if (res.succeeded()) {
          ...
          if (res.result()) {
            ...
            vertx.eventBus().<Integer>request(thisVertxName + VxApiEventBusAddressConstant.DEL_API, body, cres -> {
              if (cres.succeeded()) {
                response.end(ResultFormat.format(HTTPStatusCodeMsgEnum.C200, cres.result().body()));
  ...
  public void startAllAPI(RoutingContext rct) {
    ...
    if (StrUtil.isNullOrEmpty(appName)) {
      ...
      // 获取所有API
      vertx.eventBus().<JsonArray>request(thisVertxName + VxApiEventBusAddressConstant.FIND_API_ALL, message, data -> {
        if (data.succeeded()) {
          ...
          vertx.eventBus().<String>request(thisVertxName + VxApiEventBusAddressConstant.DEPLOY_API_START_ALL, config, option, reply -> {
            if (reply.succeeded()) {
  ...
  public void startAPI(RoutingContext rct) {
    ...
    if (StrUtil.isNullOrEmpty(apiName)) {
      ...
      vertx.eventBus().<JsonObject>request(thisVertxName + VxApiEventBusAddressConstant.GET_API, apiName, res -> {
        if (res.succeeded()) {
          JsonObject body = res.result().body();
          String appName = body.getString(VxApiDATAStoreConstant.API_APP_ID_NAME);
          JsonObject api = new JsonObject(body.getString(VxApiDATAStoreConstant.API_CONTENT_NAME));
          ...
          data.put("api", api);
          vertx.eventBus().<Integer>request(thisVertxName + VxApiEventBusAddressConstant.DEPLOY_API_START, data, reply -> {
            if (reply.succeeded()) {
  ...
  public void stopAPI(RoutingContext rct) {
    ...
    vertx.eventBus().<Integer>request(thisVertxName + VxApiEventBusAddressConstant.DEPLOY_API_STOP, body, reply -> {
      if (reply.succeeded()) {
        Integer result = reply.result().body();
  ...
  public void getTrackInfo(RoutingContext rct) {
    ...
    vertx.eventBus().<JsonObject>request(thisVertxName + VxApiEventBusAddressConstant.SYSTEM_GET_TRACK_INFO, msg, reply -> {
      if (reply.succeeded()) {
