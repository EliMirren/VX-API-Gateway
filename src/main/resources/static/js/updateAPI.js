...
function apiSerEnterInfo() {
    ...
    if (type == 'HTTP_HTTPS') {
        ...
        body.varType = $("#vars_varType").val();
        if (body.varType == 'CUSTOM') {
            if ($("#vars_varValue").val() == '') {
                alert("自定义类型: 返回值不能为空");
                isCheckTrue = false;
                return false;
            }
            body.value = $("#vars_varValue").val();
        }
        if ($("#vars_varResult").val() == '' || $("#vars_varResult").val().indexOf("${val}") < 0) {
            alert('检查到你自定义返回类型没有返回值占位符,这样该API没有任何返回意义');
        }
        body.result = $("#vars_varResult").val();
    } else if (type == 'REDIRECT') {
